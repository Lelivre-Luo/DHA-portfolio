...
export function FeaturedProjects({ className, showViewAll = true }: FeaturedProjectsProps) {
  ...
  const { featuredProjects } = getProjectsData(lang)

  // Filter out undefined projects
  const validProjects = featuredProjects.filter(project => project !== undefined)
  ...
  return (
    <div className={className}>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-3xl font-bold mb-2">{lang === 'zh' ? '精选项目' : 'Featured Projects'}</h2>
          <p className="text-muted-foreground">{lang === 'zh' ? '近期部分作品与贡献' : 'Some of my recent work and contributions'}</p>
        </div>
        {showViewAll && (
          <Link href="/projects">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {validProjects.map((project) => (
          <Card 
            key={project.id} 
            className="group overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm hover:shadow-lg transition-all duration-300 hover:border-border cursor-pointer"
