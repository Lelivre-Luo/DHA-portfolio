...
export function ResumeDownloadButton() {
  ...
  const buttonText = language === 'zh' ? '简历' : 'Resume'
  const unavailableText = language === 'zh' 
    ? '简历暂时不可用，请直接联系我。' 
    : 'Resume is currently unavailable. Please contact me directly.'

  const handleDownload = async () => {
    try {
      // 测试用：使用不存在的文件路径
      const resumePath = "/giovani-resume-test.pdf" // 不存在的文件
      ...
      if (response.ok) {
        ...
        link.download = 'Giovani_Moutinho_Resume.pdf'
